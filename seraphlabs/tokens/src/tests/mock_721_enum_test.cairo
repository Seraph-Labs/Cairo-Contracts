...
fn test_mint() {
    ...
    assert(
        Mock::token_of_owner_by_index(account, 0.into()) == 1.into(), 'token by owner index failed'
    );
}
...
fn test_owner_enum() {
    ...
    assert(
        Mock::token_of_owner_by_index(account, 1.into()) == 5.into(), 'token by owner index failed'
    );
    assert(
        Mock::token_of_owner_by_index(account, 2.into()) == 3.into(), 'token by owner index failed'
    );
    assert(
        Mock::token_of_owner_by_index(account2, 0.into()) == 2.into(), 'token by owner index failed'
    );
    assert(
        Mock::token_of_owner_by_index(account2, 1.into()) == 4.into(), 'token by owner index failed'
    );
}
