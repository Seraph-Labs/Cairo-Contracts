...
namespace JsonString {
    ...
    func append_data_to_jsonString{syscall_ptr: felt*, range_check_ptr, pedersen_ptr: HashBuiltin*}(
        arr1_len: felt, arr1: felt*, arr2_len: felt, arr2: felt*
    ) -> (arr_len: felt, arr: felt*) {
        alloc_locals;
        let (new_arr_len, new_arr) = Array.create();
    ...
    func append_data_to_enclosedString{
        syscall_ptr: felt*, range_check_ptr, pedersen_ptr: HashBuiltin*
    }(arr1_len: felt, arr1: felt*, arr2_len: felt, arr2: felt*) -> (arr_len: felt, arr: felt*) {
        alloc_locals;
        let (new_arr_len, new_arr) = Array.create();
        memcpy(new_arr, arr1, arr1_len - 1);
        memcpy(new_arr + arr1_len - 1, arr2, arr2_len);
